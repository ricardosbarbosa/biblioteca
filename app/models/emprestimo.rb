...
class Emprestimo < ActiveRecord::Base
  ...
  def status
    ...
    elsif Time.now.to_date > data_de_devolucao
      "ATRASADO"
    else
      "ALUGADO"
    end
  ...
  def validate_data_de_devolucao_deve_ser_de_no_maximo_7_dias
    if data_de_devolucao and data_de_emprestimo
      errors.add(:data_de_devolucao, "A data de devolução deve ser de no máximo 7 dias após a data de empréstimo.") if (data_de_devolucao - data_de_emprestimo) > 7.days
    end
  end
  ...
  def validate_devolvido_em_anterior_a_data_de_emprestimo
    if devolvido_em
      errors.add(:devolvido_em, "A data de devolução não pode ser anterior a data de empréstimo.")  if devolvido_em < data_de_emprestimo
    end
  end
